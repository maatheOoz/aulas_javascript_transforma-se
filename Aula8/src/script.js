...
let carro = 'Impala';
let novoCarro = carro.replace('Impala', 'Galaxie 500');

console.log(novoCarro);

// Utilização do método replaceAll()
let frase =  'Eu amo filmes. Filmes são muito legais. Filmes de drama me encantam.';

frase = frase.replaceAll("Filmes", "Séries");
frase = frase.replaceAll("filmes", "séries");
...
// Utilização do método indexOff(), lastIndexOff() e search()
let exemplo = "Por favor localize onde 'localize' ocorre.";

let exIndexOff = exemplo.indexOf('localize');

let exLastIndexOff = exemplo.lastIndexOf('localize');

let exSearch = exemplo.search('favor');

console.log(exIndexOff);
console.log(exLastIndexOff);
...
let exEndsWith = nome.endsWith('Vieira');
